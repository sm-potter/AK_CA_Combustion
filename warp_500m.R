...
raster_path <- '/mnt/data1/boreal/spotter/combustion/burned_area/final_predictions/ee/rest/500'
out <- '/mnt/data1/boreal/spotter/combustion/burned_area/final_predictions_mosaiced/ee/rest/500'
dir.create(out, recursive = T)

years <- seq(2001, 2017, 1)

bands <- c('BS', 'DEC', 'GRSH', 'JP', 'NV', 'OCON', 'WS')
bands2 <- seq(1, 7, 1)
...
foreach (band_n = bands2, .packages=c("raster")) %dopar% {
  ...
  #mosaic
  mos <- mosaic(one_ras, two_ras, three_ras, fun = mean)
