...
model_compare <- function(df, out_path, category, in_weight){
  ...
  model_quantile <- train(y ~., data = df, method = 'qrf', tuneLength = 10, metric = 'RMSE', trControl = fitControl, weights = in_weight)
  model_lasso <- train(y ~., data = df, method = 'lasso', tuneLength = 10, metric = 'RMSE', trControl = fitControl, weights = in_weight)
  model_ridge <- train(y ~., data = df, method = 'ridge', tuneLength = 10, metric = 'RMSE', trControl = fitControl, weights = in_weight)
  # model_gam <- train(y ~., data = df, method = 'bam',  tuneLength = 4, metric = 'RMSE', trControl = fitControl)
  model_svmlinear <- train(y ~., data = df, method = 'svmLinear2', tuneLength = 5, metric = 'RMSE', trControl = fitControl, weights = in_weight)
  model_svmpoly <- train(y~., data = df, method = 'svmPoly', tuneLength = 5, metric = 'RMSE', trControl = fitControl, weights = in_weight)
  model_svmradial <- train(y ~., data = df, method = 'svmRadial', tuneLength = 5, metric = 'RMSE', trControl = fitControl, weights = in_weight)
  ...
  model_quantile_final <- train(y ~., data = df, method = 'qrf', tuneGrid=data.frame(.mtry = model_quantile$bestTune$mtry), trControl = fitControl, weights = in_weight)
  model_lasso_final <- train(y ~., data = df, method = 'lasso', tuneGrid=data.frame(.fraction = model_lasso$bestTune$fraction), trControl = fitControl, weights = in_weight)
  model_ridge_final <- train(y ~., data = df, method = 'ridge', tuneGrid=data.frame(.lambda = model_ridge$bestTune$lambda), trControl = fitControl, weights = in_weight)
  # model_gam_final <- train(y ~., data = df, method = 'gamboost', tuneGrid=data.frame(.mstop = 450, .prune = 'no'), trControl = fitControl)
  model_svmlinear_final <- train(y ~., data = df, method = 'svmLinear2', tuneGrid=data.frame(.cost = model_svmlinear$bestTune$cost), trControl = fitControl, weights = in_weight)
  model_svmpoly_final <- train(y~., data = df, method = 'svmPoly', tuneGrid=data.frame(.degree= model_svmpoly$bestTune$degree, .scale = model_svmpoly$bestTune$scale, .C = model_svmpoly$bestTune$C), trControl = fitControl, weights = in_weight)
  model_svmradial_final <- train(y ~., data = df, method = 'svmRadial', tuneGrid=data.frame(.sigma = model_svmradial$bestTune$sigma, .C = model_svmradial$bestTune$C), trControl = fitControl, weights = in_weight)
  ...
  final_ranger <- train(y ~., data = df, method = 'ranger', tuneGrid=data.frame(.mtry = model_ranger$bestTune$mtry, .splitrule = model_ranger$bestTune$splitrule, .min.node.size = model_ranger$bestTune$min.node.size), weights = in_weight)
  final_svm <- train(y ~., data = df, method = 'svmRadial', tuneGrid=data.frame(.sigma = model_svmradial$bestTune$sigma, .C = model_svmradial$bestTune$C), weights = in_weight)
...
above_stand <- model_compare(above, "/mnt/data1/boreal/spotter/combustion/final_files/model_comparisons/aboveground/stand_age/rfe_weights", 'above', above_weight)
below_stand <- model_compare(below, "/mnt/data1/boreal/spotter/combustion/final_files/model_comparisons/belowground/stand_age/rfe_weights", 'below', below_weight)
# 
