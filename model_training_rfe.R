...
model_compare <- function(df, out_path, category){
  ...
  compare_ranger$density <- get_density(compare_ranger$obs, compare_ranger$pred, n = 100)
   ...
   p <- ggplot(compare_ranger)  +
    ...
    ggsave(filename = file.path(out_path,  paste0(category, 'ranger_heat_map.png')), device = 'png', dpi = 150, width = 10, height = 10)
   ...
   p <- ggplot(compare_radial)  +
     ...
     theme_bw() +
     theme(text=element_text(size=18)) + 
     ggsave(filename = file.path(out_path,  paste0(category, 'svm_heat_map.png')), device = 'png', dpi = 150, width = 10, height = 10)
  ...
  p <- ggplot(ranger_compare, aes(x = Obs, y =  Pred)) + 
    ...
    theme_bw() +
    theme(text=element_text(size=18)) + 
    ggsave(filename = file.path(out_path,  paste0(category, 'ranger_full_model_ob_pred.png')), device = 'png', dpi = 150, width = 10, height = 10)
  ...
  p2 <- ggplot(svm_compare, aes(x = Obs, y =  Pred)) + 
    ...
    theme_bw() +
    theme(text=element_text(size=18)) + 
    ggsave(filename = file.path(out_path,  paste0(category, 'svm_full_model_ob_pred.png')), device = 'png', dpi = 150, width = 10, height = 10)
...
above_stand <- model_compare(above, "/mnt/data1/boreal/spotter/combustion/final_files/model_comparisons/aboveground/stand_age/rfe_xgboost", 'above')
below_stand <- model_compare(below, "/mnt/data1/boreal/spotter/combustion/final_files/model_comparisons/belowground/stand_age/rfe_xgboost", 'below')
# 
