...
model_compare <- function(df, out_path, category){
  ...
  for_corr <- df %>% dplyr::select(-y)
  
  corr <- round(cor(for_corr), 2)
  
  #correlation cutoff of 0.6
  hc <-  findCorrelation(corr, cutoff=0.6)
  hc <-  sort(hc)
  reduced_data <-  as_tibble(data.frame(corr[,-c(hc)]))
  
  #select uncorrelated variables
 df <- df %>% dplyr::select(y, names(reduced_data))
...
above_stand <- model_compare(above, "/mnt/data1/boreal/spotter/combustion/final_files/model_comparisons/aboveground/stand_age/rfe_colinear", 'above')
below_stand <- model_compare(below, "/mnt/data1/boreal/spotter/combustion/final_files/model_comparisons/belowground/stand_age/rfe_colinear", 'below')
# 
