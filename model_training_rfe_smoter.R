...
# ggplot(below, aes(x = below.ground.carbon.combusted)) + geom_histogram()
       
#only oversample above the 90th percentile
high <- quantile(above$above.carbon.combusted, 0.90)
# above <- as_tibble(SmoteRegress(above.carbon.combusted~., data.frame(above), thr.rel = high))
above <- as_tibble(SmoteRegress(above.carbon.combusted~., data.frame(above)))

# t <- above %>% dplyr::select(above.carbon.combusted, Tree.cover) %>% arrange(desc(above.carbon.combusted))
# t2 <- above2 %>% dplyr::select(above.carbon.combusted, Tree.cover) %>% arrange(desc(above.carbon.combusted))
# 
# write_csv(t, "/mnt/data1/boreal/spotter/combustion/final_files/raw/original_above.csv")
# write_csv(t2, "/mnt/data1/boreal/spotter/combustion/final_files/raw/smote_above.csv")


high <- quantile(below$below.ground.carbon.combusted, 0.90)
below<- as_tibble(SmoteRegress(below.ground.carbon.combusted~., data.frame(below)))
...
above_stand <- model_compare(above, "/mnt/data1/boreal/spotter/combustion/final_files/model_comparisons/aboveground/stand_age/rfe_smoter", 'above')
# below_stand <- model_compare(below, "/mnt/data1/boreal/spotter/combustion/final_files/model_comparisons/belowground/stand_age/rfe_smoter", 'below')
